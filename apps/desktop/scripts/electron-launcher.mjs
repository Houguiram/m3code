...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
const APP_DISPLAY_NAME = isDevelopment ? "M3 Code (Dev)" : "M3 Code (Alpha)";
const APP_BUNDLE_ID = isDevelopment
  ? `com.houguiram.m3code.dev.${devBundleIdSuffix || "local"}`
  : "com.houguiram.m3code";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["m3code-dev"] : ["m3code"];
const LAUNCHER_VERSION = 19;
const developmentMacIconPngPath = NodePath.join(repoRoot, "assets", "m3", "m3-macos-1024.png");
const productionMacIconPngPath = NodePath.join(repoRoot, "assets", "m3", "m3-macos-1024.png");
...
export function resolveMacBundleInfoPlistStrings(executableName) {
  return {
    ...
    NSDocumentsFolderUsageDescription: "M3 Code reads project files you open in the desktop app.",
  };
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the M3 Code name.
