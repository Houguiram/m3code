...
| Quota snapshot      | Remaining CodexBar subscription windows (session / weekly / plan) for Claude, Codex, and Cursor, joined onto provider instances by email or a Settings pin. Distinct from the Usage page's local token/cost scan. See [usage](../user/usage.md). |
