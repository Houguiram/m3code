...
For a GitHub pull request, **Open in Graphite** opens the same review in Graphite. It sits beside
the host's own link in Git actions, in a review's menu, and on a right-click of a pull request
number. Enable it for a project in **Settings → Project → Pull requests**. If the repository uses
Graphite's merge queue, enable **Graphite merge queue** and enter the exact GitHub label configured
in Graphite. The primary pull request action then adds or removes that label; direct GitHub merge
methods stay in the actions menu.
